...
| F1-based model comparison | Forecasting-specific replacement required | Forecast evaluation needs time-aware target/horizon metrics and baselines. |
...
| Classification feature schema and preprocessing | Irrelevant | Marvel character features, category normalization, and LightGBM encodings do not match time-series forecasting inputs. |
...
| Endpoint load-test utilities and token extraction | Irrelevant and unsafe pattern | No endpoint is proposed; embedded token handling should not be reproduced. |
...
| Operational code excluded from linting | Excluded | Thin operational scripts are production code and must be checked. |
...
| Weekly Monday production schedule copied verbatim | Insufficient evidence | Forecast cadence is a product decision; schedules remain configurable and paused. |
